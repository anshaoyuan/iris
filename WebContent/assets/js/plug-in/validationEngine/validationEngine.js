 ...
 * 表单验证插件
 * 
 * 具体配置用法请参看  https://github.com/posabsolute/jQuery-Validation-Engine
 * 
...
//	require('plug-in/validationEngine/validationEngine.css');
	...
	var methods = {
		...
		attach : function(userOptions) {
			...
			if (options.binded) {
				...
				form.find("[" + options.validateAttribute + "*=validate]").not(
						...
						".datepicker").bind(options.validationEventTrigger,
		...
		detach : function() {
			...
			// unbind fields
			form.find("[" + options.validateAttribute + "*=validate]").not(
					"[type=checkbox]").unbind(options.validationEventTrigger,
					methods._onFieldEvent);
			form
					.find(
							"["
			...
			form.find("[" + options.validateAttribute + "*=validate]").not(
					"[type=checkbox]").die(options.validationEventTrigger,
					methods._onFieldEvent);
			form.find(
					"[" + options.validateAttribute
		...
		validate : function() {
			...
			if (element.is("form") && !element.hasClass('validating')) {
				...
				// If the form doesn't validate, clear the 'validating' class
				// before the user has a chance to submit again
				setTimeout(function() {
		...
		_validateFormWithAjax : function(form, options) {
			...
			var type = (options.ajaxFormMethod) ? options.ajaxFormMethod
					: "GET";
			var url = (options.ajaxFormValidationURL) ? options.ajaxFormValidationURL
					: form.attr("action");
			var dataType = (options.dataType) ? options.dataType : "json";
			
			if (options.duplicateSubmit) {
				 if (options.submit) {
			...
			$.ajax({
						...
						// options);
						// },
						error : options.onAjaxFaild,
						success : function(json) {
							if (json && (dataType == "json") && (json !== true)) {
		...
		_validateField : function(field, options, skipAjaxValidation) {
			...
			for ( var i = 0, field_errors = 0; i < rules.length; i++) {
				...
				case "notequals":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
							rules, i, options, methods._notequals);
					break;
				case "funcCall":
					errorMsg = methods._getErrorMessage(form, field, rules[i],
		...
		_notequals : function(field, rules, i, options) {
			...
			if (field.val() == $("#" + notequalsField).val())
		...
		_ajax : function(field, rules, i, options) {
			...
			if (!options.isError
				...
				data[field.attr('name')] = data.fieldValue;
				$
						.ajax({
							type : 'POST',
							url : rule.url,
							...
							error : function() {
								var errorField = $("#" + data.fieldId).eq(0);
								methods._showPrompt(errorField,
										rule.alertTimeOut, "", true, options);
							},
							success : function(json) {
		...
		_showPrompt : function(field, promptText, type, ajaxed, options,
			...
			if (ajaxform)
				prompt = false;
			if ($.trim(promptText)) {
				field = field.attr('delegate') ? field.parents('form').find(
						field.attr('delegate')) : field;
				if (prompt)
	...
	function init(selector, options, method) {
		(function($) {
...
//							"regex" : /^([\+][0-9]{1,3}[ \.\-])?([\(]{1}[0-9]{2,6}[\)])?([0-9 \.\-\/]{3,20})((x|ext|extension)[ ]?[0-9]{1,4})?$/,
							"regex" :/^(13[0-9]\d{8}|15[0-9]\d{8}|14[0-9]\d{8}|18[0-9]\d{8}|0[1-9]\d{1,2}[-][1-9]\d{6,7})$/,
//							"regex" :/^0?(13[0-9]|15[0-9]|18[0-9]|14[57])[0-9]{8}$/,
							"alertText" : "* 无效的电话号码"
						},
						"email" : {
						...
						"qqmsn" : {
							"regex" : /^(((([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+(\.([a-z]|\d|[!#\$%&'\*\+\-\/=\?\^_`{\|}~]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])+)*)|((\x22)((((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(([\x01-\x08\x0b\x0c\x0e-\x1f\x7f]|\x21|[\x23-\x5b]|[\x5d-\x7e]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(\\([\x01-\x09\x0b\x0c\x0d-\x7f]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF]))))*(((\x20|\x09)*(\x0d\x0a))?(\x20|\x09)+)?(\x22)))@((([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|\d|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.)+(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])|(([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])([a-z]|\d|-|\.|_|~|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])*([a-z]|[\u00A0-\uD7FF\uF900-\uFDCF\uFDF0-\uFFEF])))\.?)|^[1-9]\d{4,10}$/,
							"alertText" : "* qq或msn无效"
						},
						"integer" : {
							"regex" : /^[\-\+]?\d+$/,
						...
						"ajaxValidateTeamName" : {
							"url" : "webTeam!ajaxValidateTeamName.action",
							"alertTextOk" : "* 此帐号名称可以使用",
							"alertText" : "* 此名称已被其他人使用",
							"alertTextLoad" : "*  后台处理，请稍等。。"
						},
						"ajaxValidateTeamNameForUpdate" : {
							"url" : "webTeam!ajaxValidateTeamNameForUpdate.action",
							"alertTextOk" : "* 此帐号名称可以使用",
							"alertText" : "* 此名称已被其他人使用",
							"alertTextLoad" : "*  后台处理，请稍等。。"
						},
						"ajaxValidateVestName" : {
							"url" : "webset!ajaxCheckVestName.action",
							"alertTextOk" : "* 该马甲名可以使用",
							"alertText" : "* 该马甲名已被其他人使用",
							"alertTextLoad" : "*  后台处理，请稍等。。"
						},
						"ajaxValidateRssName" : {
							"url" : "webRssAction!ckeckRssName.action",
							"alertTextOk" : "* 该RSS名可以使用",
							"alertText" : "* 该RSS名已被其他人使用",
							"alertTextLoad" : "*  后台处理，请稍等。。"
						},
						"ajaxValidateRssUrl" : {
							"url" : "webRssAction!ckeckRssUrl.action",
							"alertTextOk" : "* 该RSSUrl可以使用",
							"alertText" : "* 该RSSUrl已被其他人使用",
							"alertTimeOut" : "* 该RSSUrl已不可用或网络错误",
							"alertTextLoad" : "*  后台处理，请稍等。。"
						},
						...
						"ajaxApplyRegiste" : {
							"url" : "userRegiste!ajaxApplyRegiste.action",
							// you may want to pass extra data on the ajax call
							"extraData" : "name=eric",
							"alertText" : "* 此名称已被其他人使用",
							"alertTextLoad" : "* 后台处理，请稍等。"
						...
						// 验证密码是否正确
						"ajaxUserPassword" : {
							"url" : "webset!ajaxCheckPassword.action",
							"extraData" : "name=eric",
							"alertText" : "* 原始密码不正确",
							"alertTextLoad" : "* 正在验证密码是否正确，请稍等。"
						},
	...
	$.validationEngine = {
		...
		defaults : {
			...
			onAjaxFaild : function(data, transport) {
